...
/**
 * The method that is responsible for all processes. It calls methods from
 * blurring.ImageBlurrer to blur, binarize and use custom blurring. The output
...
public class RunProgram {
	public static void main(String args[]) throws IOException {
		...
		if(args.length == 3) {
			...
			String path_label = args[2];
			
			String[] tmp_arr = path_label.split("\"");
			
			String[] arr_path_label = tmp_arr[1].split(",");
			
			String inputFilename = arr_path_label[0];
			int inputLabel = 0;
			try{
				inputLabel = Integer.parseInt(arr_path_label[1]);
			...
			int whatSet = train; // 'test' for testing set of 7500+ snippets. 'train'
								// for 400 training snippets
			boolean outCustom = true; // Set true output the image, false when
										// output of images themselves is not
										// necessary
			boolean outBinary = true; // Set true to output binary images
			boolean outBlurred = true; // Set true to output blurred images.
			boolean outAdjusted = true; // Set true to output adjusted images.
			...
			/*
			 * July 23rd 2014: Recent Changes results in the code no longer using
			 * the first argument for depth since we need all three depths for
			 * generating a 6x3 attribute dataset.
			 * 
			 * All the following methods are repreted on two other images with diff
			 * blur levels to generate statistics. In approach 2, since only one is
			 * needed, the other two images and its processes are skipped.
			 */
			...
			/*
			 * Calls to methods that blur, binarize and custom blur followed by
			 * feature extraction.
			 */
	
			// ---------------------------------------------------------------------------
	
//			imb.adjustImage(img5, outAdjusted);
//			imb.blurImage(img5 ,outBlurred);
			...
			/*
			 * creating an instance of the class responsible for the analysis file
			 * output.
			 */
			...
			if (blurMode == tripleRegular) {
				...
				afw.writeSingleLine(bni5); // writes the stats to the respective
											// *.arff file
			}
		}
	}
	...
	/**
	 * This method imports the image whose filepath is passed as a parameter and
	 * returns an models.Image object.
	 * 
	 * @param inputFilename
	 * @return models.Image
	 */
	public static Image importImage(String inputFilename) {
		...
		img.setHorizontal(img.getByteImage()[0].length);
		img.setVertical(img.getByteImage().length);
		return img;
	}
}
